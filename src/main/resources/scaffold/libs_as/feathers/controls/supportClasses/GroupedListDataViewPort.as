	...
	public class GroupedListDataViewPort extends FeathersControl implements IViewPort
	{
		...
		private function refreshSelection() : void
		{
			for each( var item : DisplayObject in this._layoutItems )
			{
				...
				if( itemRenderer )
				{
					itemRenderer.isSelected = itemRenderer.groupIndex == this._selectedGroupIndex && itemRenderer.itemIndex == this._selectedItemIndex;
				}
			}
		}
		...
		private function refreshRenderers() : void
		{
			if( this._typicalItemRenderer )
			{
				if( this._typicalItemIsInDataProvider )
				{
					var itemStorage : ItemRendererFactoryStorage = this.factoryIDToStorage( this._typicalItemRenderer.factoryID , this._typicalItemRenderer.groupIndex , this._typicalItemRenderer.itemIndex );
		...
		private function findRendererForItem( item : Object , groupIndex : int , itemIndex : int , layoutIndex : int ) : void
		{
			...
			if( itemRenderer )
			{
				...
				{
					var storage : ItemRendererFactoryStorage = this.factoryIDToStorage( itemRenderer.factoryID , itemRenderer.groupIndex , itemRenderer.itemIndex );
		...
		private function renderUnrenderedData() : void
		{
			...
			for( var i : int = 0; i < rendererCount; i += 3 )
			{
				...
				var itemRenderer : IGroupedListItemRenderer = this.createItemRenderer( item , groupIndex , itemIndex , layoutIndex , true , false );
				this._layoutItems[ layoutIndex ] = DisplayObject( itemRenderer );
			}

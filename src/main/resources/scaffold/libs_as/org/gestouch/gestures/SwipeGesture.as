package org.gestouch.gestures
{
	...
	public class SwipeGesture extends AbstractDiscreteGesture
	{
		...
		override protected function onTouchMove( touch : Touch ) : void
		{
			...
			if( _noDirection )
			{
				if( (offsetLength > slop || slop != slop) && (avrgVel >= minVelocity || offsetLength >= minOffset) )
				{
			...
			else
			{
				...
				if( absVelX > absVelY )
				{
					...
					{
						if( (recentOffsetX < 0 && (direction & SwipeGestureDirection.LEFT) == 0) || (recentOffsetX > 0 && (direction & SwipeGestureDirection.RIGHT) == 0) || Math.abs( Math.atan( _offset.y / _offset.x ) ) > ANGLE )
						{
				...
				else if( absVelY > absVelX )
				{
					...
					{
						if( (recentOffsetY < 0 && (direction & SwipeGestureDirection.UP) == 0) || (recentOffsetY > 0 && (direction & SwipeGestureDirection.DOWN) == 0) || Math.abs( Math.atan( _offset.x / _offset.y ) ) > ANGLE )
						{
							// movement in opposite direction
							// or too much diagonally
